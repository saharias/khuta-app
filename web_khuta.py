...
# --- مرحلة التنظيف (التعديل المطلوب: بالمتر) ---
stage1 = st.checkbox("☑ التنظيف والتعقيم", value=True)
if stage1:
    c1, c2 = st.columns(2)
    clean_per_meter = c1.number_input("سعر تنظيف المتر الواحد:", value=7.0, step=0.5)
    clean_lab = c2.number_input("عمالة إضافية (اختياري):", value=0.0)
    calculated_clean_cost = (clean_per_meter * area) + clean_lab
    st.caption(f"تكلفة التنظيف الفعلية: {calculated_clean_cost:.2f} ريال")
...
if st.button("احسب التسعيرة النهائية 🚀", type="primary", use_container_width=True):
    ...
    if stage1:
        cost1 = (clean_per_meter * area) + clean_lab
        total_cost += cost1
...
================================""", language="text")
    ...
    with tab2:
        st.warning("تقرير الإدارة الخاص")
        st.write(f"**إجمالي التكلفة:** {total_cost:.2f} ريال")
        st.write(f"**صافي الربح:** {net_profit:.2f} ريال")
